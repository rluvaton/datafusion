...
use arrow::array::{Array, ArrayRef, BooleanBufferBuilder, GenericListArray, OffsetSizeTrait, as_generic_list_array, new_empty_array};
...
pub struct ListGroupValueBuilder<const FIXED_BLOCK_SIZING: bool, O: OffsetSizeTrait> {
    field: FieldRef,
    offsets: BlockedOffsetBufferBuilder<FIXED_BLOCK_SIZING, O>,
    child: Box<dyn GroupColumn<false>>,
    outer_nulls: BlockedNullsBuilder<FIXED_BLOCK_SIZING>,
    outer_len: usize,
}

impl<const FIXED_BLOCK_SIZING: bool, O: OffsetSizeTrait> ListGroupValueBuilder<FIXED_BLOCK_SIZING, O> {
    pub fn new(field: FieldRef, block_size: usize, child: Box<dyn GroupColumn<false>>) -> Self {
        Self {
            field,
    ...
    fn push_offset(&mut self, additional: usize) -> Result<bool> {
        let additional_o = O::from_usize(additional).ok_or_else(|| {
            internal_datafusion_err!(
                "List sublist length {additional} overflows the offset type"
            )
        })?;
        if self.current_end().checked_add(&additional_o).is_none() {
            return internal_err!(
        ...
        Ok(self.offsets.push_length(additional))
    }
    ...
    fn equal_to_typed(
        ...
        let lhs_end = self.offsets[lhs_row.next_index_in_block()].as_usize();
        let lhs_len = lhs_end - lhs_start;
        if lhs_len != rhs_sublist.len() {
            return false;
        }
        for j in 0..lhs_len {
            // having the same block
            if !self.child.equal_to(lhs_row.add_index_in_block(j), &rhs_sublist, j) {
                return false;
            }
        }
        true
    ...
    fn append_val_typed(
        ...
        if array.is_null(row) {
            ...
            let should_create_new_block = self.offsets.push_length(0);
            if FIXED_BLOCK_SIZING && should_create_new_block {
            ...
            let should_create_new_block = self.push_offset(n)?;
            if FIXED_BLOCK_SIZING && should_create_new_block {
                self.child.start_new_block()
            }
        }
...
impl<const FIXED_BLOCK_SIZING: bool, O: OffsetSizeTrait> GroupColumn<FIXED_BLOCK_SIZING> for ListGroupValueBuilder<FIXED_BLOCK_SIZING, O> {
    ...
    fn take_block(&mut self) -> Option<ArrayRef> {
        ...
        let child = self.child.take_block().unwrap_or_else(|| {
    ...
    fn start_new_block(&mut self) {
        assert!(!FIXED_BLOCK_SIZING, "you can only start block if block size is externally managed");
...
//     }
//
//     fn make_child() -> Box<dyn GroupColumn<false>> {
...
//             Some(vec![]),
//         ]);
//         let mut builder = ListGroupValueBuilder::<true, i32>::new(child_field(), DEFAULT_BLOCK_SIZE, make_child());
...
//             Some(vec![Some(10)]),
//         ]);
//         let mut builder = ListGroupValueBuilder::<true, i64>::new(child_field(), DEFAULT_BLOCK_SIZE, make_child());
//         for i in 0..input.len() {
//             builder.append_val(&input, i).unwrap();
//         }
//         let out = Box::new(builder).build();
...
//         ]);
//
//         let mut per_row = ListGroupValueBuilder::<true, i32>::new(child_field(), DEFAULT_BLOCK_SIZE, make_child());
//         for i in 0..input.len() {
//             per_row.append_val(&input, i).unwrap();
//         }
//
//         let mut vec_b = ListGroupValueBuilder::<true, i32>::new(child_field(), DEFAULT_BLOCK_SIZE, make_child());
...
//         let builder = ListGroupValueBuilder::<true, i32>::new(child_field(), DEFAULT_BLOCK_SIZE, make_child());
//         let out = Box::new(builder).take_block().expect("must have block for empty list");
//         let out = out.as_any().downcast_ref::<ListArray>().unwrap();
//         assert_eq!(out.len(), 0);
//     }
//

...
impl BlockedBytesBufferBuilder {
  pub fn new() -> Self {
    ...
    let memory = blocks.capacity() * size_of::<Vec<u8>>() + blocks[0].allocated_size();
  ...
  pub(crate) fn extends_bytes_from_offsets_indexes_in_current_block<O: OffsetSizeTrait>(&mut self, bytes: &[u8], offset_buffer_slice: &[O], indexes: &[usize]) {
    let block = self.blocks.back_mut().unwrap();
  ...
  pub fn take_block(&mut self) -> Option<Vec<u8>> {
    ...
    self.memory -= (self.blocks.capacity() - before) * size_of::<Vec<u8>>();
    self.memory -= current_block.as_ref().map(|block| block.capacity()).unwrap_or(0);

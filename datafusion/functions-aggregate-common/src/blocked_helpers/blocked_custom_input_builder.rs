...
#[derive(Debug)]
pub struct BlockedCustomInputBuilder<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider: BlockProvider> {
...
impl<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider: BlockProvider> BlockedCustomInputBuilder<FIXED_BLOCK_SIZING, CustomBlockProvider> {
  ...
  pub fn new(block_size: usize, blocks_provider: CustomBlockProvider) -> Self {
    ...
    let memory = blocks.capacity() * size_of::<CustomBlockProvider::Block>() + blocks[0].allocated_size();
  ...
  pub fn start_new_block(&mut self) {
    ...
    self.memory += (new_capacity - prev_capacity) * size_of::<CustomBlockProvider::Block>();
  }
  ...
  pub(crate) fn reserve_blocks(&mut self, n: usize) {
    ...
    self.memory += (self.blocks.capacity() - prev_capacity) * size_of::<CustomBlockProvider::Block>();
  }
  ...
  ) -> bool where CustomBlockProvider::Block: BlockWithSlice {
    let mut block = &mut self.blocks[self.current_block_index];
  ...
  pub fn extend_from_slice(&mut self, mut buffer: &[<CustomBlockProvider::Block as Block>::Item]) where CustomBlockProvider::Block: BlockWithSlice {
  ...
  pub(super) fn current_block_remaining_len(&self) -> usize {
    assert!(FIXED_BLOCK_SIZING, "current block remaining length is only relevant for manual block size");
    self.block_size - self.blocks[self.current_block_index].len()
  }

  pub(crate) fn push_value_n_within_block(&mut self, value: <CustomBlockProvider::Block as Block>::Item, n: usize) -> bool where CustomBlockProvider::Block: BlockWithSlice {
    self.len += n;
    let mut block = &mut self.blocks[self.current_block_index];
    ...
    if FIXED_BLOCK_SIZING {
      ...
      assert!(new_len <= self.block_size, "overflow from block new block length: {new_len}, block size: {}", self.block_size);
    }
  ...
  /// Push default
  pub fn push_default_n(&mut self, n: usize) where CustomBlockProvider::Block: BlockWithSlice, <CustomBlockProvider::Block as Block>::Item: Default + Clone {
    self.push_value_n(<CustomBlockProvider::Block as Block>::Item::default(), n);
  }

  pub fn push_value_n(&mut self, value: <CustomBlockProvider::Block as Block>::Item, mut n: usize) where CustomBlockProvider::Block: BlockWithSlice, <CustomBlockProvider::Block as Block>::Item: Clone {
    // If not fixed, then treat all offsets as single block
    if !FIXED_BLOCK_SIZING {
  ...
  pub fn take_block(&mut self) -> Option<CustomBlockProvider::Block> {
    ...
    self.memory -= (self.blocks.capacity() - prev_blocks_capacity) * size_of::<CustomBlockProvider::Block>();
  ...
  pub fn take_block_finished(&mut self) -> Option<CustomBlockProvider::FinishedBlock> where CustomBlockProvider: BlockProviderFinish {
  ...
  pub fn reset(&mut self) {
    ...
    self.number_of_blocks = 1;
    self.pending_block = false;
    self.memory = self.blocks.capacity() * size_of::<CustomBlockProvider::Block>() + self.blocks[0].allocated_size();
  }
}

impl<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider: BlockProvider> Extend<<CustomBlockProvider::Block as Block>::Item> for BlockedCustomInputBuilder<FIXED_BLOCK_SIZING, CustomBlockProvider> {
  fn extend<T: IntoIterator<Item = <CustomBlockProvider::Block as Block>::Item>>(&mut self, iter: T) {
    if !FIXED_BLOCK_SIZING {
      self.extend_in_block(
        iter.into_iter(),
        true,
      );
...
impl<CustomBlockProvider> Index<usize> for BlockedCustomInputBuilder<true, CustomBlockProvider>
where
  CustomBlockProvider: BlockProvider,
  CustomBlockProvider::Block: Index<usize, Output = <CustomBlockProvider::Block as Block>::Item> {
...
impl<CustomBlockProvider> IndexMut<usize> for BlockedCustomInputBuilder<true, CustomBlockProvider>
where
  CustomBlockProvider: BlockProvider,
  CustomBlockProvider::Block: IndexMut<usize, Output = <CustomBlockProvider::Block as Block>::Item> {

  fn index_mut(&mut self, index: usize) -> &mut Self::Output {
    self.index_mut(BlocksIndex::from_index_in_fixed_block_size(index, self.block_size))
  }
}

impl<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider> Index<BlocksIndex> for BlockedCustomInputBuilder<FIXED_BLOCK_SIZING, CustomBlockProvider>
where
  CustomBlockProvider: BlockProvider,
  CustomBlockProvider::Block: Index<usize, Output = <CustomBlockProvider::Block as Block>::Item> {
  type Output = <CustomBlockProvider::Block as Block>::Item;
...
impl<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider> IndexMut<BlocksIndex> for BlockedCustomInputBuilder<FIXED_BLOCK_SIZING, CustomBlockProvider>
where
  CustomBlockProvider: BlockProvider,
  CustomBlockProvider::Block: Index<usize, Output = <CustomBlockProvider::Block as Block>::Item>,
  CustomBlockProvider::Block: IndexMut<usize, Output = <CustomBlockProvider::Block as Block>::Item> {

...
impl<const FIXED_BLOCK_SIZING: bool> BlockedBooleanBuilder<FIXED_BLOCK_SIZING> {
  pub fn new(block_size: usize) -> Self {
    ...
    let allocated_size = blocks.capacity() * size_of::<BooleanBufferBuilder>() + allocated_size_for_builder(&blocks[0]);
  ...
  pub(crate) fn start_new_block(&mut self) {
    ...
    self.allocated_size += (self.blocks.capacity() - capacity_before) * size_of::<BooleanBufferBuilder>();
  }
  ...
  pub fn set_bit(&mut self, blocked_index: BlocksIndex, is_set: bool) {
    self.blocks[blocked_index.block_index()].set_bit(blocked_index.index_in_block(), is_set)
  }
  ...
  pub fn take_block(&mut self) -> Option<BooleanBuffer> {
    ...
    self.allocated_size -= (capacity_before - self.blocks.capacity()) * size_of::<BooleanBufferBuilder>();
...
impl<const MANUAL_BLOCK_SIZE: bool> Extend<bool> for BlockedBooleanBuilder<MANUAL_BLOCK_SIZE> {
...
impl<const FIXED_BLOCK_SIZING: bool> Index<BlocksIndex> for BlockedBooleanBuilder<FIXED_BLOCK_SIZING> {
  type Output = bool;

  fn index(&self, blocked_index: BlocksIndex) -> &Self::Output {
    if self.blocks[blocked_index.block_index()].get_bit(blocked_index.index_in_block()) {
      &true
    } else {
      &false
    }

...
#[derive(Debug)]
pub struct BlockedCustomInputBuilderWithLifetime<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider: BlockWithLifetimeProvider> {
...
impl<const FIXED_BLOCK_SIZING: bool, CustomBlockProvider: BlockWithLifetimeProvider> BlockedCustomInputBuilderWithLifetime<FIXED_BLOCK_SIZING, CustomBlockProvider> {
  ...
  pub fn push(&mut self, value: <CustomBlockProvider::Block as BlockWithLifetime>::Item<'_>) -> bool {
  ...
  pub(super) fn extend_in_block<'a>(
    &mut self,
    iter: impl Iterator<Item = <CustomBlockProvider::Block as BlockWithLifetime>::Item<'a>> + 'a,
  ...
  ) -> bool where CustomBlockProvider::Block: BlockWithLifetimeWithSlice {
    let mut block = &mut self.blocks[self.current_block_index];
  ...
  pub fn extend_from_slice(&mut self, mut buffer: &[<CustomBlockProvider::Block as BlockWithLifetime>::Item<'_>]) where CustomBlockProvider::Block: BlockWithLifetimeWithSlice {
  ...
  pub(crate) fn push_value_n_within_block(&mut self, value: <CustomBlockProvider::Block as BlockWithLifetime>::Item<'_>, n: usize) -> bool where CustomBlockProvider::Block: BlockWithLifetimeWithSlice {
    self.len += n;
    let mut block = &mut self.blocks[self.current_block_index];
  ...
  /// Push default
  pub fn push_default_n<'a>(&mut self, n: usize) where CustomBlockProvider::Block: BlockWithLifetimeWithSlice, <CustomBlockProvider::Block as BlockWithLifetime>::Item<'a>: Default + Copy {
    self.push_value_n(<CustomBlockProvider::Block as BlockWithLifetime>::Item::default(), n);
  }

  pub fn push_value_n<'a>(&mut self, value: <CustomBlockProvider::Block as BlockWithLifetime>::Item<'a>, mut n: usize) where CustomBlockProvider::Block: BlockWithLifetimeWithSlice, <CustomBlockProvider::Block as BlockWithLifetime>::Item<'a>: Copy {
    // If not fixed, then treat all offsets as single block
    if !FIXED_BLOCK_SIZING {
  ...
  pub fn take_block_finished(&mut self) -> Option<CustomBlockProvider::FinishedBlock> where CustomBlockProvider: BlockProviderWithLifetimeFinish {
  ...
  pub fn value(&self, index: BlocksIndex) -> <CustomBlockProvider::Block as BlockWithLifetime>::Item<'_> {

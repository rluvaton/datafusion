...
use arrow::array::{GenericBinaryArray, GenericStringArray, NullBufferBuilder};
    ...
    V: Debug + PartialEq + Eq + Clone + Copy + Default,
{
        ...
        for i in 0..values.len() {
            ...
            if values.is_null(i) {
                let payload = if let Some(&(payload, _offset)) = self.null.as_ref() {
                    ...
                    let null_index = BlocksIndex::new(self.views.num_blocks() - 1, self.views.current_block_len());
    ...
    pub fn take_block(&mut self) -> Option<ArrayRef> {
        let num_blocks = self.num_buffer_blocks_per_block.pop_front().expect("must have block");
        let buffers = (0..num_blocks).map(|_| self.buffer.take_block_finished().expect("must have block")).collect::<Vec<_>>();
        ...
        let (views, null_buffer) = match (views, null_buffer) {
            ...
            (Some(_), None) => unreachable!("views have block while nulls did not"),
            (None, Some(_)) => unreachable!("null have block while views do not"),
        };
        ...
        if self.views.len() == 0 {
            ...
            self.map.retain(|entry| {
                if let Some(new_block_index) = entry.start_block_index.checked_sub(num_blocks) {
                    entry.start_block_index = new_block_index;
                    true
                } else {
                    false
                }
    ...
    pub fn size(&self) -> usize {
        ...
        let in_progress_size = self.buffer.allocated_size();
        let completed_size = self.buffer.allocated_size();

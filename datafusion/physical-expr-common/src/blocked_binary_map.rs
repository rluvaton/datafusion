...
use datafusion_expr_common::groups_accumulator::enumerate_blocked::EnumerateBlocked;
    ...
    V: Debug + PartialEq + Eq + Clone + Copy + Default,
{
        ...
        for (value, &hash) in values.iter().zip(batch_hashes.iter()) {
            ...
            let Some(value) = value else {
                let payload = if let Some(&(payload, _offset)) = self.null.as_ref() {
                    ...
                    let block_index = self.buffer.num_blocks() - 1;
                    let null_index = BlocksIndex::new(block_index, self.offsets.current_block_len());
            ...
            // value is not "small"
                ...
                let entry = self.map.find_mut(hash, |header| {
                    ...
                    let existing_value =
                        unsafe { self.buffer.block(header.block_index).get_unchecked(header.range()) };
    ...
    pub fn take_block(&mut self) -> Option<ArrayRef> {
        ...
        let values = self.buffer.take_block_finished()?;
        ...
        if self.offsets.len() == 0 {
            ...
            self.map.retain(|entry| {
                if let Some(new_block_index) = entry.block_index.checked_sub(1) {

...
pub(crate) struct MinMaxStructBlockedAccumulator {
    /// Inner data storage.
    inner: BlockedMinMaxStructState,
    /// if true, is `MIN` otherwise is `MAX`
    is_min: bool,
}

impl MinMaxStructBlockedAccumulator {
    pub fn new_min(data_type: DataType, block_size: usize) -> Self {
        Self {
            inner: BlockedMinMaxStructState::new(data_type, block_size),
            is_min: true,
        }
    }
...
impl BlockedGroupsAccumulator for MinMaxStructBlockedAccumulator {
    fn update_batch(
        &mut self,
        values: &[ArrayRef],
...
#[derive(Debug)]
struct BlockedMinMaxStructState {
    /// The minimum/maximum value for each group
    min_max: BlockedVecBuilder<true, Option<StructArray>>,
...
impl BlockedMinMaxStructState {
    ...
    fn emit_to(&mut self, emit_to: EmitTo) -> (usize, Vec<Option<StructArray>>) {
        ...
        if self.min_max.len() == 0 {
            ...
            self.min_max = BlockedVecBuilder::new(block_size);
        }
...
mod tests {
    ...
    fn test_min_max_simple_struct() {
        ...
        let mut min_accumulator =
            MinMaxStructBlockedAccumulator::new_min(array.data_type().clone());
        let mut max_accumulator =
            MinMaxStructBlockedAccumulator::new_max(array.data_type().clone());

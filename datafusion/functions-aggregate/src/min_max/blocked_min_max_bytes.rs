...
use datafusion_expr::groups_accumulator::BlocksIndex;
...
impl MinMaxBytesBlockedAccumulator {
    ...
    pub fn new_min(data_type: DataType, block_size: usize) -> Self {
        Self {
            inner: BlockedMinMaxBytesState::new(data_type),
            is_min: true,
            block_size,
        }
    }
...
impl GroupsAccumulator for MinMaxBytesBlockedAccumulator {
    fn update_batch(
        &mut self,
        values: &[ArrayRef],
        group_indices: &[usize],
...
struct BlockedMinMaxBytesState {
    ...
    min_max: BlockedVecBuilder<true, Option<Vec<u8>>>,
